...
func Clear(list *List) error {
	...
	list.Length = 0
	return clearNode(list.Node, list.Node)
}
func clearNode(node *Node, listNode *Node) error {
	if nil == node || nil == listNode || node == listNode {
		return nil
	}
	if nil == node.Value {
		return nil
	...
	return clearNode(newNode, listNode)
}
...
func Insert(list *List, i int, e interface{}) error {
	...
	curNode := &Node{
		Value:     e,
		RightNode: newNode.RightNode,
		LeftNode:  newNode,
	}

	newNode.RightNode = curNode
	newNode.RightNode.LeftNode = curNode
	list.Length += 1
...
func Delete(list *List, i int) (interface{}, error) {
	...
	newNode.RightNode = newNode.RightNode.RightNode
