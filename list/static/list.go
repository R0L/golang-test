package static

import "C"
import (
	"errors"
)

const (
	DefaultMaxSize          = 20
	ErrOutOfChainList       = "out of chain list"
	ErrChainListAlreadyFull = "chain list already full"
	ErrChainListIsEmpty     = "chain list is nil"
)

//线性表的数据对象集合为(a1,a2,...,a3)，每个元素的类型均为DataType。其中，除第一个元素a1外，每一个元素有且只有一个直接前驱元素，除了最后一个元素an外，每一个元素有且只有一个直接后继元素。数据元素之间的关系是一对一的关系。
// 节点
type List [DefaultMaxSize]struct {
	Index int
	Value interface{}
}

// 初始化操作，建立一个空的线性表L。
func Init() *List {
	var list = &List{}
	for i := 0; i < DefaultMaxSize; i++ {
		list[i].Index = i + 1
	}
	list[DefaultMaxSize-1].Index = 0
	return list
}

// 若线性表为空，返回true，否则返回false。
func Empty(list *List) (bool, error) {
	if nil == list {
		return false, errors.New(ErrChainListIsEmpty)
	}

	if 0 == list[DefaultMaxSize-1].Index {
		return true, nil
	}

	return false, nil
}

// 将线性表清空。
func Clear(list *List) error {
	if nil == list {
		return errors.New(ErrChainListIsEmpty)
	}
	for i := 0; i < DefaultMaxSize; i++ {
		list[i].Index = i + 1
		list[i].Value = nil
	}
	list[DefaultMaxSize-1].Index = 0
	return nil
}

// 将线性表L中的第i个位置元素值返回给e。
func Get(list *List, i int) (interface{}, error) {
	var findVal interface{}
	if nil == list {
		return nil, errors.New(ErrChainListIsEmpty)
	}
	if i <= 0 {
		return nil, errors.New(ErrOutOfChainList)
	}
	curIndex := DefaultMaxSize - 1
	for j := 0; j < DefaultMaxSize; j++ {
		if i == j {
			findVal = list[curIndex].Value
			break
		}
		curIndex = list[curIndex].Index
		if 0 == curIndex {
			return nil, errors.New(ErrOutOfChainList)
		}
	}

	return findVal, nil
}

// 在线性表L中查找与给定值e相等的元素，如果查找成功，返回该元素在比表中序号表示成功；否则，返回0表示失败。
func Locate(list *List, e interface{}) (int, error) {
	var findKey int
	if nil == list {
		return findKey, errors.New(ErrChainListIsEmpty)
	}
	curIndex := DefaultMaxSize - 1
	for j := 0; j < DefaultMaxSize; j++ {
		if e == list[curIndex].Value {
			findKey = j
			break
		}
		curIndex = list[curIndex].Index
	}

	return findKey, nil
}

// 在线性表L中的第i个位置插入新元素e。
func Insert(list *List, i int, e interface{}) error {
	if nil == list {
		return errors.New(ErrChainListIsEmpty)
	}

	if Length(list) >= DefaultMaxSize-2 {
		return errors.New(ErrChainListAlreadyFull)
	}

	if i <= 0 {
		return errors.New(ErrOutOfChainList)
	}

	curIndex := DefaultMaxSize - 1
	for j := 0; j < DefaultMaxSize; j++ {
		if i == j+1 {
			useIndex := list[0].Index
			list[0].Index = list[useIndex].Index

			list[useIndex].Index = list[curIndex].Index

			list[curIndex].Index = useIndex
			list[useIndex].Value = e
			break
		}
		curIndex = list[curIndex].Index
		if 0 == curIndex {
			return errors.New(ErrOutOfChainList)
		}
	}

	return nil
}

// 删除线性表L中第i个位置元素，并用e返回其值。
func Delete(list *List, i int) (interface{}, error) {
	var retVal interface{}
	if nil == list {
		return nil, errors.New(ErrChainListIsEmpty)
	}
	if 0 >= i {
		return nil, errors.New(ErrOutOfChainList)
	}

	curIndex := DefaultMaxSize - 1
	for j := 1; j <= DefaultMaxSize; j++ {
		if i == j+1 {
			delIndex := list[curIndex].Index
			list[curIndex].Index = list[delIndex].Index
			list[delIndex].Index = list[0].Index
			retVal = list[delIndex].Value
			list[delIndex].Value = nil
			list[0].Index = delIndex
			break
		}
		curIndex = list[curIndex].Index
		if 0 == curIndex {
			return retVal, errors.New(ErrOutOfChainList)
		}
	}

	return retVal, nil
}

// 返回线性表L的元素个数。
func Length(list *List) int {
	var length int
	if nil == list {
		return 0
	}
	curIndex := DefaultMaxSize - 1
	for i := 0; i < DefaultMaxSize; i++ {
		if 0 == list[curIndex].Index {
			length = i
			break
		}
		curIndex = list[curIndex].Index
	}
	return length
}

// 遍历
func ToArray(list *List) []interface{} {
	var ret []interface{}
	curIndex := DefaultMaxSize - 1
	for i := 1; i <= DefaultMaxSize; i++ {
		curIndex = list[curIndex].Index
		if 0 == curIndex {
			break
		}
		ret = append(ret, list[curIndex].Value)
	}
	return ret
}
